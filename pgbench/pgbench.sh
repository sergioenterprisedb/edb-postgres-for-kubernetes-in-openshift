...
create_table() {
  get_primary
  ${kubectl_cmd} exec -it ${primary} -- psql -U postgres -c \
...
# Get primary instance
get_primary() {
  export primary=`kubectl get pod -n ${namespace} -o=jsonpath="{range .items[*]}{.metadata.name}{'\t'}{.status.podIP}{'\t'}{.metadata.labels.role}{'\n'}" | grep ${cluster_name} | grep primary | awk '{print $1}'`
}

# Show results
show_results() {
  get_primary
  ${kubectl_cmd} exec -it ${primary} -- psql -U postgres -c "select * from pgbench_test"
}
...
NAMESPACE=${namespace}  # Change to the namespace where your job is running
